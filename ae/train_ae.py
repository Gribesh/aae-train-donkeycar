...
def main():
    ...
    parser.add_argument(
        "-ae", "--ae-path", help="Path to saved autoencoder (otherwise start from scratch)", type=str)
    parser.add_argument(
    ...
    parser.add_argument(
        "--n-epochs", help="Number of epochs", type=int, default=10)
    ...
    for folder in args.folders:
        ...
        images_ = [
            folder + im for im in os.listdir(folder) if im.endswith(".jpg")]
    ...
    if args.ae_path is not None:
        ...
        autoencoder = Autoencoder(
            z_size=args.z_size, learning_rate=args.learning_rate)
    ...
    try:
        for epoch in range(args.n_epochs):
            ...
            if args.verbose >= 1:
                ...
                r = ROI
                im = image[int(r[1]): int(r[1] + r[3]),
                           int(r[0]): int(r[0] + r[2])]
                # Resize if needed
                if ROI[2] != INPUT_DIM[1] or ROI[3] != INPUT_DIM[0]:
                    im = cv2.resize(
                        im, (INPUT_DIM[1], INPUT_DIM[0]), interpolation=cv2.INTER_AREA)
